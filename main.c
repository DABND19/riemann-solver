...
bool is_flow_equals(const GasParameters* left, const GasParameters* right) {
...
int main(int argc, char** argv) {
  ...
  const GasParameters LEFT = {atof(argv[1]), atof(argv[2]), atof(argv[3])};
  const GasParameters RIGHT = {atof(argv[4]), atof(argv[5]), atof(argv[6])};
  ...
  for (size_t i = 0; i < CELLS_NUM; ++i) {
    ...
    GasParameters flow = x < X_DIAPH ? LEFT : RIGHT;
    u1[i] = U1(&flow);
    u2[i] = U2(&flow);
    u3[i] = U3(&flow);
  }
  ...
  double t = 0;
  while (t < T) {
    GasParameters left_boundary_flow, right_boundary_flow;
    ...
    for (size_t i = 0; i < CELLS_NUM - 1; ++i) {
      GasParameters left_flow = from_conservative(u1[i], u2[i], u3[i]);
      GasParameters right_flow = from_conservative(u1[i + 1], u2[i + 1], u3[i + 1]);

      GasParameters knot_flow;
      ...
      if (is_flow_equals(&left_flow, &right_flow)) {
        ...
        max_velocity = gsl_max(max_velocity, -left_wave_velocity(&exact_solution));
        max_velocity = gsl_max(max_velocity, right_wave_velocity(&exact_solution));
      }
    }
  ...
  for (size_t i = 0; i < CELLS_NUM; ++i) {
    double x_left = X_LEFT + i * dx;
    double x_right = x_left + dx;
    GasParameters flow = from_conservative(u1[i], u2[i], u3[i]);
    GasParameters flow_ = solution_on_curve(&exact, 0.5 * (x_left + x_right) / T);
